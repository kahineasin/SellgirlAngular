...
import { ngettext, msgid } from './pf_ttag';
...
type FormattedString = string | any;
...
// function formatWeek(m: Dayjs, options: FormattingOptions = {}) {
...
export function getRemappedValue(
  ...
  if (remap && column) {
    ...
    if (
      (column as any).hasRemappedValue &&
      (column as any).hasRemappedValue(value)
    ) {
      // $FlowFixMe: column could be Field or Column
      return (column as any).remappedValue(value);
    }
    // or it may be a raw column object with a "remapping" object
    if (
      (column as any).remapping instanceof Map &&
      (column as any).remapping.has(value)
    ) {
      return (column as any).remapping.get(value);
    }
...
export function formatField(field: FieldClass): string {
  if (!field) {
    return '';
