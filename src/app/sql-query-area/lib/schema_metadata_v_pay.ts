...
import { t } from "./pf_ttag";
import { isa, isFK as isTypeFK, isPK as isTypePK, TYPE } from "./types";
...
export function getOperatorByTypeAndName(type, name) {
  ...
  //   }
  // );
  const typedNamedOperator = FILTER_OPERATORS_BY_TYPE_ORDERED[type].filter(
    (a) => a.name === name
  );
...
function summableFields(fields) {
  ...
  return fields.filter(isSummable);
}

function scopeFields(fields) {
  //return _.filter(fields, isScope);
  return fields.filter(isScope);
}
  ...
  return AGGREGATION_OPERATORS.filter(
    (aggregationOperator) =>
      !(
        ...
        //   aggregationOperator.requiredDriverFeature
        // )
        table.db.features.indexOf(aggregationOperator.requiredDriverFeature) < 0
      )
...
export function getAggregationOperator(short) {
  ...
  return AGGREGATION_OPERATORS.filter((a) => a.short === short);
}
...
export function addValidOperatorsToFields(table) {
  for (const field of table.fields) {
    field.filter_operators = getFilterOperators(field, table, null);
  }
