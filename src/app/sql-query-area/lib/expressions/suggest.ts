...
export function suggest({
  ...
  if (
    ...
    ((isTokenType(lastInputToken.tokenType, Identifier) &&
      (Identifier.PATTERN as any).test(
        partialSource[partialSource.length - 1]
      )) ||
  ...
  const context = getContext({
    source: null, //metabase原版本没有这句,但编译不通过
    cst,
    tokenVector,
    targetOffset,
    startRule,
...
export class ExpressionContextVisitor extends ExpressionCstVisitor {
  private targetOffset: any;
  private tokenVector: any;
  private stack: any;
