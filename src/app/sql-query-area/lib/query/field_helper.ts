...
export function getFieldClause(fields: FieldReference[]): FieldsClause {
  fields = getFields(fields as any);
  if (fields.length === 0) {
    return undefined;
  } else {
    return fields as FieldsClause;
  }
}
...
export function removeField(fields: FieldsClause, index: number): FieldsClause {
...
export function getFieldValues(field: Field): FieldValues {
  const values: any = field && field.values;
  if (Array.isArray(values)) {
    ...
    if (Array.isArray(values.human_readable_values)) {
      return _.zip(values.values, values.human_readable_values || {}) as any;
...
export function getRemappings(field: Field) {
  const remappings = (field && (field as any).remappings) || [];
