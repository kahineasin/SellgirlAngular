...
export const IFRAMED_IN_SELF = (function () {
  try {
    return window.self !== window.top && tmpWindow.top.METABASE;
  } catch (e) {
    return false;
  }
})();
...
export function getSelectionPosition(element): number[] {
  // input, textarea, IE
...
export function shouldOpenInBlankWindow(
  ...
  let tmpEvent: any = event;
  if (blank) {
    return true;
  } else if (
    blankOnMetaKey &&
    (event && tmpEvent.metaKey != null ? tmpEvent.metaKey : metaKey)
  ) {
    return true;
...
export function initializeIframeResizer(readyCallback = () => {}) {
  ...
  if (tmpWindow.iFrameResizer) {
    ...
    // let tmpRequire: any = require;//去掉这里不知道有没有问题--benjamin todo
    // if (typeof tmpRequire.ensure !== "function") {
    //   return false;
    // }
