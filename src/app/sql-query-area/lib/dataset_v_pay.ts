 ...
 */
export function fieldRefForColumn(column): any {
...
export const keyForColumn = (column) => {
  ...
  if (dimension instanceof FieldDimension) {
    dimension = dimension.withoutOptions("join-alias") as unknown as Dimension;
  }
...
export function findColumnIndexForColumnSetting(columns, columnSetting) {
  ...
  return columns.findIndex((col) => col.name === columnSetting.name);
}
    ...
    for (const columnSetting of columnSettings) {
      if (columnSetting.enabled) {
        ...
        if (columnSetting.fieldRef) {
          ...
          const index = columnNames.findIndex((n) => n === columnSetting.name);
          if (index >= 0) {
        ...
        if (fieldRef) {
          ...
          if (
            ...
            (dimension as any).joinAlias()
          ) {
            const join = (dimension as any).join();
            if (join) {
