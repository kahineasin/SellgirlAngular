...
//   DataTableModel,
//   DataTableUIModel,
// } from "../../../../core/model/data-integration";
import {
  DatabaseModel,
  DataColumnModel,
  DataTableModel,
...
export class LimitStepComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //   return r;
    // };
    if (!me.pfUtil.isNull(me.query.limit)) {
      me.limitNum = me.query.limit;
    }
  }
  ngOnChanges() {
    const me = this;
  ...
  public onLimitChange(event) {
    const me = this;
    me.query.limit = event;
  }
  ...
  //   // let tmpTable = me._fieldList.find((a) => a.key.ShortId === table.ShortId);
  //   // if (tmpTable !== null) {
  //   //   tmpTable.value = tmpTable.value.filter(
  //   //     (a) => a.ShortId !== column.ShortId
  //   //   );
  //   // }
  //   // for (let i = me._fieldList.length - 1; i >= 0; i--) {
  //   //   me._fieldList[i].value = me._fieldList[i].value.filter(
