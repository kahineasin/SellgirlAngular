...
import { TableId } from '../../../../model/Table';
// import type {
//   Join as JoinObject,
//   JoinStrategy,
//   JoinFields,
...
export default class Join extends MBQLObjectClause {
  ...
  setAlias(alias: JoinAlias): any {
    ...
    if (alias !== this.alias) {
      ...
      if (
        ...
        let mbql = joinDimension.mbql();
        (mbql as any)[1] = alias;
        return join.setJoinDimension(mbql as any);
      } else {
        return join;
      }
    }
  ...
  setDefaultAlias() {
    ...
    if (parentDimension && parentDimension.field().isFK()) {
      return this.setAlias((parentDimension.field() as any).targetObjectName());
    } else {
  ...
  strategyOptions() {
    ...
    return JOIN_STRATEGY_OPTIONS.filter(({ value }) =>
      database.hasFeature(value as any)
    );
  }
  ...
  setParentDimension(dimension: Dimension | ConcreteField): Join {
    if (dimension instanceof Dimension) {
      dimension = dimension.mbql() as any;
    }
    const joinDimension = this.joinDimension();
    return this.setCondition([
      '=',
      dimension,
      joinDimension instanceof Dimension ? (joinDimension.mbql() as any) : null,
    ]);
  }
  ...
  setJoinDimension(dimension: Dimension | ConcreteField): Join {
    ...
    return this.setCondition([
      '=',
      parentDimension instanceof Dimension
        ? (parentDimension.mbql() as any)
        : null,
      dimension,
    ]);
  }
  ...
  joinedDimensions() {
    ...
    return table
      ? (table as any)
          .dimensions()
          .map((dimension) => this.joinedDimension(dimension))
      : [];
  }
