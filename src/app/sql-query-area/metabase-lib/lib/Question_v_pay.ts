...
import StructuredQueryClass, {
  STRUCTURED_QUERY_TEMPLATE,
} from "./queries/StructuredQueryClass";
// import NativeQuery, {
//   NATIVE_QUERY_TEMPLATE,
// } from "./queries/NativeQuery";
import AtomicQuery from "./queries/AtomicQuery";
...
import { memoize, sortObject } from "./utils";
...
export default class Question {
  ...
  query(): Query {
    ...
    for (const QueryClass of [StructuredQueryClass, NativeQuery, InternalQuery]) {
  ...
  isStructured(): boolean {
    return this.query() instanceof StructuredQueryClass;
  }
  ...
  setDefaultDisplay(): Question {
    ...
    const query = this.query();
    if (query instanceof StructuredQueryClass) {
  ...
  syncColumnsAndSettings(previous, queryResults) {
    ...
    if (
      query instanceof StructuredQueryClass &&
      previousQuery instanceof StructuredQueryClass
    ) {
  ...
  topLevelQuestion(): Question {
    const query = this.query();
    if (query instanceof StructuredQueryClass && query !== query.topLevelQuery()) {
  ...
  objectDetailPK(): any {
    const query = this.query();
    if (this.isObjectDetail() && query instanceof StructuredQueryClass) {
...
(window as any).Question = Question;
(window as any).NativeQuery = NativeQuery;
(window as any).StructuredQuery = StructuredQueryClass;
